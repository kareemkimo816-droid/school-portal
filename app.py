...
if stage != "Choose Grade / اختر المرحلة":
    ...
    try:
        # --- التعديل الجوهري هنا ---
        # استخدام رابط التصدير المباشر (export) مع تحديد اسم الورقة (sheet) 
        # هذا الرابط يجلب "كامل" محتويات الصفحة ولا يتجاهل أي صفوف قديمة
        ...
        if not df.empty:
            # ترتيب عكسي: الأحدث (اللي تحت في الشيت) يظهر أول واحد في الموقع
            # ده بيضمن إن 5/3 و 1/3 يظهروا فوق و 28/2 يظهر تحتهم
            ...
            st.warning(f"No data found for {stage}. جرب إضافة بيانات في الشيت.")
            
    except Exception as e:
        st.error(f"Error! تأكد من أن اسم التبويب في جوجل شيت هو '{stage}' بالضبط.")
